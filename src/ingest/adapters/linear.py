"""Linear adapter. Real field shapes confirmed from
vendor/EnterpriseRAG-Bench/generated_data/sources/linear/**/*.json."""
...
class LinearAdapter(SourceAdapter):
    ...
    def build_document(self, rec: dict, file_path: Path) -> Document:
        ...
        return Document(
            ...
            author_raw=rec.get("creator"),
            thread_key=None,
            uri=f"linear://{rec.get('team')}/{rec.get('key')}",
            content_hash=hashlib.sha256(body.encode()).hexdigest(),
            simhash=compute_simhash(body),
            declared_container=rec.get("team"),
        )
