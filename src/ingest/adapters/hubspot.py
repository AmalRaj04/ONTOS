"""HubSpot adapter. Real field shapes confirmed from
vendor/EnterpriseRAG-Bench/generated_data/sources/hubspot/**/*.json. Each record is a
company/deal, not a single interaction — title_field_name is "company_name"."""
...
class HubspotAdapter(SourceAdapter):
    ...
    def build_document(self, rec: dict, file_path: Path) -> Document:
        ...
        return Document(
            ...
            thread_key=None,
            uri=f"hubspot://{rec.get('company_id')}",
            content_hash=hashlib.sha256(body.encode()).hexdigest(),
            simhash=compute_simhash(body),
            declared_container=rec.get("stage"),
        )
