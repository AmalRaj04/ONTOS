"""Fireflies adapter. Real field shapes confirmed from
vendor/EnterpriseRAG-Bench/generated_data/sources/fireflies/**/*.json."""
...
class FirefliesAdapter(SourceAdapter):
    ...
    def build_document(self, rec: dict, file_path: Path) -> Document:
        ...
        return Document(
            ...
            title=get_title(rec),
            body=body,
            created_at=parse_iso_date(rec.get("recorded_at")),
            author_raw=rec.get("redwood_owner"),
            thread_key=None,
            uri=f"fireflies://{rec.get('meeting_id')}",
            content_hash=hashlib.sha256(body.encode()).hexdigest(),
            simhash=compute_simhash(body),
            declared_container=rec.get("customer_company"),
        )
